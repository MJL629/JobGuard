...
async def _handle_job_recommendation(db, user_id, session_id):
    ...
    try:
        ...
        # 限制匹配数量，避免超时
        jobs_to_match = jobs[:15]
        yield _sse_event("status", {"stage": "matching", "message": f"正在匹配 {len(jobs_to_match)} 个岗位..."})
        
        matches = await job_matcher.match_batch(user_profile, jobs_to_match, top_k=5)
        if not matches:
            msg = "暂时没找到特别匹配的岗位，你可以上传简历或告诉我更多偏好。"
            yield _sse_event("message", {"content": msg})
            _save_assistant_message(db, session_id, msg)
            return
        ...
        for i, m in enumerate(matches[:5], 1):
            ...
            lines.append(f"{i}. {j.get('job_title')} @ {j.get('company_name')} - 匹配度 {s}%")

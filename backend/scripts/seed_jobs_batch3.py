"""
批量岗位种子数据灌入脚本
读取 data/seed_jobs_batch3.json，写入 MySQL
"""
import json
import os
import sys
...
def load_jobs() -> list[dict]:
    path = os.path.join(os.path.dirname(__file__), "..", "data", "seed_jobs_batch3.json")
...
def seed_jobs(db: Session):
    jobs = load_jobs()
    print(f"准备灌入 {len(jobs)} 条岗位数据 (Batch 3)...")
    ...
    for item in jobs:
        ...
        source_url = item.get("source_url", f"batch3_{company_name}_{job_title}")
